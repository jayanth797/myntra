  ...
  return (
    <View style={[styles.container, { backgroundColor: currentColors.background }]}>
      <View style={[styles.header, { backgroundColor: currentColors.background, borderBottomColor: isDark ? '#333' : '#f0f0f0' }]}>
        <Text style={[styles.headerTitle, { color: currentColors.text }]}>Categories</Text>
      </View>

      <View style={[styles.searchContainer, { backgroundColor: currentColors.background, borderBottomColor: isDark ? '#333' : '#f0f0f0' }]}>
        <View style={[styles.searchInputContainer, { backgroundColor: isDark ? '#333' : '#f5f5f5' }]}>
          <Search size={20} color={currentColors.icon} style={styles.searchIcon} />
          <TextInput
          ...
          {searchQuery !== "" && (
            <TouchableOpacity onPress={clearSearch}>
              <X size={20} color={currentColors.icon} />
            </TouchableOpacity>
          )}
        </View>
      </View>
            ...
            {filtercategories?.map((category: any) => (
              ...
              >
                ...
                <View style={styles.categoryInfo}>
                  <Text style={[styles.categoryName, { color: currentColors.text }]}>{category.name}</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View style={styles.subcategories}>
                      {category?.subcategory?.map((sub: any, index: any) => (
                        <TouchableOpacity
                          key={index}
                          style={[styles.subcategoryTag, { backgroundColor: isDark ? '#333' : '#f5f5f5' }]}
                          onPress={() => handleSubcategorySelect(sub)}
                        >
                          <Text style={[styles.subcategoryText, { color: currentColors.text }]}>{sub}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </ScrollView>
