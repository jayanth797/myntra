...
// Import static orders (simulate import)
// Since we can't easily import non-exported consts, we rely on the file scope
// Note: This assumes `getOrders` returns a promise resolving to an array
    ...
    const fetchorder = async () => {
      if (user) {
        ...
        try {
          ...
          console.log("API failed, loading local orders");
          const localOrders = await getOrders();

          if (localOrders.length > 0) {
            setorder(localOrders);
          } else {
            // Fallback to local orders even if empty to avoid crash
            setorder(localOrders);
          }
        } finally {
          setIsLoading(false);
